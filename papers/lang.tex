...
$f$ must exist. However the edge structure of $G'$ and $G_{min}$ need not be the same, as Figure
\ref{fig:bijection_counterexample} demonstrates.

\begin{figure}
\label{fig:bijection_counterexample}
...
\begin{lemma}
If $(p,q) \in E(G')$ then $(f(p),f(q)) \in E(G_{min})$
\end{lemma}
\begin{proof}
...
\begin{theorem}
$(p,q) \in E(G')$ if and only if $(f(p),f(q)) \in E(G_{min})$
\end{theorem}
...
At each outgoing edge node add a new episilon transition to an accepting state.
