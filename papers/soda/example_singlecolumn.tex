...
Since the orbit indices are equal and $\mathcal{H_G}(G_1)=\mathcal{H_G}(G_2)$, then $\mathcal{H_N}(v_1, G_1)=\mathcal{H_N}(v_2, G_2)$. \qed
\end{proof}
...
\node[font=\small] at (0,-8.7) {unfolding $\mathcal{T}(G,a)$};
\end{tikzpicture}
...
The color of a node in round $i$ is exactly the Merkle-hash of an unfolding tree with a maximum depth of $i+1$ layers. Note here that our unfolded finite tree is being built bottom-up, with a new layer added to the tree every iteration. Due to the bottom-up construction, the hashing process maximally shares computation, making this a dynamic programming approach. Note that we differ here from classic color refinement by operating over directed rather than undirected graphs. Our approach also draws inspiration from DFA partition refinement strategies, such as Hopcraft's DFA minimization algorithm \cite{hopcraft-redescribe,hopcraft}.
...
\begin{algorithm}[t]
    ...
    \Function{$\mathcal{H_L}$}{$G=(V, E, L)$}
        ...
        \State reachable $\gets \{n \mapsto \textproc{reachable}(Q, n) \mid n \in V'\}$
        \State stable $\gets \{\}$
...
\begin{enumerate}
  ...
  This particular formulation is very similar to that of nondeterminstic finite automata, except that characters are accumulated in a string by visiting nodes (instead of traversing edges), and all states can be terminal states.
  ...
  \item As a \textit{bisimulation system}. The approach given in Section \ref{sec:local-graph-hashing} seeks to hash the unfolding, which is a slight twist on the philosophy of bisimulation. In a typical bisimulation setup, we say that nodes $a$ and $b$ are bisimilar if they have the same labels and if the following two conditions are met:
  \begin{itemize}
  ...
  \end{itemize}
  Note the use of the existential operator instead of a bijective mapping between successor nodes. This means that bisimulation inherently ignores duplicate bisimilar states. For example, two nodes may be bisimilar but have different out-degrees if mulitple successor nodes are themselves bisimilar. Extending computation of a node's hash modulo bisimilarity instead of tree expansion requires only a small modification to the previously described procedure: when computing the quotient graph $G/C$, collapse the graph to an ordinary digraph instead of a multi-digraph.
\end{enumerate}
...
The core of the digraph Merkle hashing algorithm was independently discovered by the developers of the Unison programming language for use in hashing abstract syntax trees of the Unison language. However, their approach does not take into account computation of the orbits of the nodes, and 
