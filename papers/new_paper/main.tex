...
\begin{algorithm}[t]
    \caption{Local Digraph Node Hash}\label{alg:local-digraph-node-hash}
    \begin{algorithmic}[1]
    \Function{$\mathcal{H_N}$}{$G=(V, E, L)$}
          ...
          \For{$n \in V'$}
            \If{$n \notin \text{stable}$}
              \If{\textproc{partition\_unchanged}(reachable[n], colors, new\_colors)}
                ...
                \State final\_hash[n] $\gets$ \textproc{serialize}($Q$, $n$, reachable[n], new\_colors)
              \EndIf
            \EndIf
          \EndFor
        \EndWhile
