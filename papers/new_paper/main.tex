...
\begin{itemize}
  \item \textbf{Global Hashing} - for use cases where the \textit{global} structure of the digraph is of importance. A correct algorithm here should produce equal hashes if and only if two graphs are isomoprhic. Hashing a molecular structure (in the chemistry sense) is one example use case where the global structure is important.
...
One common theme throughout this paper is reliance on conversion of graphs to \textit{canonical form}. If two graphs (or nodes within) can be converted to a canonical form, we can serialize the canonical form and pass it through an ordinary string function. We should additionally take care to distinguish between the hash of an entire graph vs the hash of a specific node within a graph. Our general strategy for computing the hash of a specific node is to combine the overall graph hash with the node's position in canonical order. Care must be taken to ensure that our index accounts for any symmetry present in the graph (ie, two nodes may be identical modulo automorphism).
...
It may come as a suprise that there are many different ways of defining the hash of a graph or the hash of the nodes contained within a graph. The definition of a graph hash therefore changes the algorithmic implementation needed to compute the hash values. To explore the design space of node hashing, we need to determine which properties we can \textit{observe} when traversing the graph. Some properties of interest that we may consider in our algorithm design include:

\begin{enumerate}
  \item When we are visiting a node, can we observe the presence of outgoing edges? Can we observe the presence of incoming edges?
  \item Can we observe the number of outgoing and/or incoming edges?
  \item After visiting a node and beginning a traversal, can we determine if we have visited a node previously in the walk? Can we observe the same for edges? Can we determine when we previously visited the node or edge?
\end{enumerate}

In this paper we will first cover an algorithm for computing hashes when we can observe maximally global properties. In other words, nodes will hash to the same value only for graphs that are exactly isomorphic. We will then explore an algorithm for hashing with a minimal amount of observable properties. This will be accomplished by viewing the hash of a node as the hash of a particular type of language generated by the walks through the graph.

\subsection{Maximally Global Graph Hashing}
