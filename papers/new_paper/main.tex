...
\begin{algorithm}
    \caption{Global Directed Graph Hash}\label{digraphhash}
    \begin{algorithmic}[1]
...
\begin{algorithm}
    \caption{Global Directed Graph Node Hash}\label{alg:digraph-node-hash}
    \begin{algorithmic}[1]
...
We now define the following stabilization condition which will tell us when to stop computing new colors on the next step of our algorithm:

\textbf{Local Stabilization Condition}: A node $n$ is stable in round $i$ if for all nodes $u$, $v$ in the reachable subgraph from $n$, $\lambda_{i+1}(u)=\lambda_{i+1}(v)$ if and only if $\lambda_i(u)=\lambda_i(v)$.

We then define the canonical label of a node $n$ to be the coloring of $n$ in the round in which local stabilization in the quotient graph is achieved for $n$. The canonical label can be used to order the nodes of the quotient graph. For some input node $n$ that we are hashing, we then say that the hash of $n$ is equal to the hash of the adjacency list of the subgraph reachable subgraph from $n$, sorted in canonical order via the canonical labels.
