...
\begin{itemize}
  \item \textbf{Global Hashing} - for use cases where the \textit{global} structure of the digraph is of importance. A correct algorithm here should produce equal hashes if and only if two graphs are isomorphic.
  \item \textbf{Local Hashing} - for use cases where observers can only observe \textit{local} information by traversing from a node to a neighbor on an outgoing edge. There are several formulations of local hashing, but in this paper we will discuss an algorithm that produces equal hashes if and only if the (potentially infinite) tree expansions from outward exploration are isomorphic. Local hashing has many commonalities with bisimulation and co-inductive representations of finite directed graphs.
  \item \textbf{Ordered Outgoing Edges} - in many practical applications the outgoing edges from a node are ordered. We can use this additional structure to simplify and improve the runtime performance of our hashing algorithms.
...
The color of a node in round $i$ is exactly the hash of a tree expansion with a maximum depth of $i+1$ layers. Note here that our finite tree is being built bottom-up, with a new layer added to the tree every iteration. Due to the bottom-up construction, the hashing process maximally shares computation, making this a dynamic programming approach.

Our approach differs from classic color refinement in that we are operating over directed graphs rather than undirected graphs. We note here inspiration and similarities to partition refinement strategies as well, such as Hopcraft's DFA minimization algorithm \cite{hopcraft-redescribe,hopcraft}.
...
The strategy employed for local hashing depends on properties we can observe by traversing the graph forward along its directed edges. Here are two other formulations of local hashing which we will note here:
...
\begin{enumerate}
  ...
  This particular formulation is very similar to that of nondeterminstic finite automata, except that characters are added by visiting nodes (instead of traversing edges), and all states can be terminal states.
  ...
  \item As a \textit{bisimulation system}. The approach given in Section \ref{sec:local-graph-hashing} seeks to hash the tree expansion, which is a slight twist on the philosophy of bisimulation. In pure bisimulation hash, we say that nodes $a$ and $b$ are bisimilar if they have the same labels and if the following two conditions are met:
  \begin{itemize}
  ...
  \end{itemize}
  Note the use of ``there exists'', which implies that bisimulation ignores the arity of outgoing edges that the tree expansion formulation takes into account.
  
  \,

  Computing the hash of a system modulo bisimilarity should therefore be a small modification - when computing the quotient graph $G/C$, we instead want to receive a digraph instead of a multi-digraph.
\end{enumerate}

\section{Outgoing Ordered Edge Graphs}

For many real world applications of graph hashing, the labels of outgoing edges can be strictly ordered.  We can take advantage of this extra structure on the edges to create simpler hashing algorithms.
...
The core of the digraph Merkle hashing algorithm was independently discovered by the developers of the Unison programming language for use in hashing abstract syntax trees. However, their approach does not take into account computation of the orbits of the nodes, and 
