...
by computing the hash of a node's \textit{vertex language}. This type of language is
highly reminiscient of how we can define regular languages using finite automata.

We define the directed vertex language $\mathcal{L}_G(u)$ of some node $u$ in directed graph
$G$ to be the set of all strings formed by all walks starting from $u$ in $G$. The strings
in this set are built from the labels of the vertices visited in each walk. This is in contrast
to a DFA where the characters are added by walking along an edge. More formally,
the language of a node can be recursively defined as follows:

\begin{equation}
  \mathcal{L}_G(u) = \{L(u)\} \cup \bigcup_{v \in \text{neighbor}(u)} \{L(u) \circ x \mid x \in \mathcal{L}_G(v)\}
\end{equation}
