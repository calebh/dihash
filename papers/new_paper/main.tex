...
\begin{itemize}
  ...
  \item \textbf{Digraph Merkle Hashing} - for use cases where we want graphs to be hashed recursively, or graphs are structures that are built up over time. In this approach we hash strongly connected components individually using either global or local hashing, then use recursion on the acyclic condensation graph induced by the strongly connected components.
...
\begin{inlineproof}
  ...
  ($\Leftarrow$) Let $V_1$ be the vertices of the quotient graph $Q_1 = G_1 / C_1$ reachable from $u$ and $V_2$ be the vertices of the quotient graph $Q_2 = G_2 / C_2$ reachable from $v$.
  
  Then it is easy to construct a bijection of the vertices in the tree expansions since they are isomorphic. This bijection maps between nodes with isomorphic subtrees. Since the quotient graph merges any nodes with identical tree expansions, this same bijection can be re-interpreted as a bijection $f : V_1 \rightarrow V_2$ between the nodes in the reachable portions of the quotient graph.

  We now show that $f$ also preserves edges. We prove the following cases:
  \begin{itemize}
    \item If $(a, b) \in E_1$, then $(f(a), f(b)) \in E_2$. Assume that $(f(a), f(b)) \notin E_2$. Then $\textproc{tree\_expansion}(G_1, a) \ncong  \textproc{tree\_expansion}(G_1, f(a))$, which contradicts the assumption that $f$ also maps between isomorphic tree expansions.
    \item If $(f(a), f(b)) \in E_2$, then $(a, b) \in E_1$. Assume that $(a, b) \notin E_1$. Then $\textproc{tree\_expansion}(G_1, a) \ncong  \textproc{tree\_expansion}(G_1, f(a))$, which contradicts the assumption that $f$ also maps between isomorphic tree expansions.
  \end{itemize}

  We now show that if $(a, b) \in E_1$, then $(f(a), f(b)) \in E_2$.
\end{inlineproof}


\clearpage

In contrast to a global hashing approach, a local hashing strategy depends on the properties we can observe by traversing the graph along edges. A local graph hash can be formulated in several ways:

\begin{enumerate}
  \item As a \textit{directed vertex language}. In this formulation, the hash of a node depends on the (potentially infinite) set of strings that can be formed by visiting labeled nodes. This particular formulation is very similar to that of nondeterminstic finite automata, except that characters are added by visiting nodes (instead of edges), and all states can be terminal states.
  
  Our primary note on this formulation of local graph hashing is that computing a hash value is PSPACE-hard. For this reason, we will not elaborate much further on solving this problem as an efficient algorithm is not possible.

  \textbf{Proof Sketch:} We perform a poly-time reduction from the NFA universality problem (UNIV), which is PSPACE-complete \cite{univ}. We say that an NFA is universal if it accepts every string. Given an input NFA, we can perform a poly-time version to an equivalent directed vertex language. We can then construct a universal NFA and encode it in a directed vertex language. If we could compute the hash of a directed vertex language, we could decide the equality between the input NFA and the universal NFA.
\end{enumerate}
...
\section{Local Vertex Hashing}

In contrast to global graph hashing, local graph hashing computes a vertex hash based on solely local properties that can be observed by walking through the graph. In our formulation, the local hash of a vertex is defined as the hash of the potentially infinite tree formed by traversing the directed graph. This particular formulation is that of bisimulation equivalence and is useful for hashing finite co-inductive formulations of directed graphs.

Our approach begins by computing a quotient graph where nodes that are bisimilar are merged into a single node. This can be computed by a color refinement algorithm. Although most commonly used as a first pass phase in graph isomorphism checks, color refinement actually computes groups of nodes that are bisimilar.

Once a quotient graph modulo bisimilarity is computed of our input graph, we then proceed to compute a canonical order of the input nodes. This can be accomplished by computing the hash of a finite tree expansion for each node. The depth of the finite tree expansion should be set to the minimum depth needed to distinguish any two nodes in the reachable part of the quotient graph. To optimize the computation of the hash, the tree should be built bottom-up. In practice, computing the quotient graph and performing this canonization step can be fused together and calculated simultaneously.

Suppose that we have graph $G$ minimized to $G'$ and some arbitrary minimal $G_min$. We then show the existance of a bijection $f : V(G') \rightarrow V(G_{min})$ when Tree($G$) = Tree($G_{min}$).

\begin{lemma}
  $f$ is defined everywhere.
\end{lemma}
\begin{proof}
  Consider some vertex $p$ in $G$. Let $T=\text{Tree}_{G'}(p)$. Then $T \in \text{Tree}(G')$. However $\text{Tree}(G')=\text{Tree}(G_{min})$. This means that there must be a vertex $p'$ such that $T=\text{Tree}_{G_{min}}(p')$. We can then map $p \mapsto p'$ in $f$.
\end{proof}
...
\begin{proof}
  ...
  Assume $f$ is not well defined. Then there is some $p$ such that $f(p)=p'$ and $f(p)=p''$ where $p' \neq p''$.

  This means that $\text{Tree}_{G_{min}}(p)=\text{Tree}_{G_{min}}(p'')$ which means that $p$ and $p''$ have equal tree expansions. This contradicts the assumption that $G_{min}$ is minimal.
\end{proof}
...
\end{lemma}
\begin{proof}
  Assume that $f$ is not injective. Then there are some states $p$ and $q$ such that $f(p)=f(q)$.

  Since $\text{Tree}(G)=\text{Tree}(G_{min})$, this implies that $p$ and $q$ have identical tree expansions. However our algorithm verified that $p$ and $q$ had different tree expansions, which is a contradiction.
\end{proof}
...
\end{lemma}
\begin{proof}
  Assume $f$ is not surjective. Then there is some state $p'$ such that there is no node $p$ such that $f(p)=p'$. This implies that $\text{Tree}(G') \neq \text{Tree}(G_{min})$ which is a contradiction.
\end{proof}
...
This idea is essentially that of bisimulation equivalence and is useful for hashing co-inductive formulations of directed graphs.

For our formulation of local vertex hashing, the only property we can observe while traversing the graph are vertex labels. Other properties, such as vertex out-degree are not observable properties for our purposes.

With these ideas in mind, we can write a canonization modulo bisimulation equivalence by using a color refinement algorithm:

\begin{enumerate}
  \item Given an input graph $G$ and a vertex $v$, remove all nodes in $G$ that are not reachable from $v$.
  \item 
\end{enumerate}

Note here that in our formulation, we cannot explicitly observe other properties such as vertex out degree.




We will now explore the design space for hashing a node in a directed graph by 


\section{Directed Vertex Languages}

We will now explore the design space for hashing a node in the directed graph
by computing the hash of a node's \textit{vertex language}. We define the directed vertex
language $L_G(u)$ of some node $u$ in directed graph
$G$ to be the set of all strings formed by all walks starting from $u$ in $G$. The strings
in this set are built from the labels of the vertices visited in each walk. This differs from a
more standard deterministic finite automaton (DFA) where characters are added to a string by
walking along an edge. More formally, we can describe the language of a node using recursion
and a union operation:

\begin{equation}
  L_G(u) = \{\mathcal{L}(u)\} \cup \bigcup_{v \in \text{neighbor}(u)} \{\mathcal{L}(u) \circ x \mid x \in L_G(v)\}
\end{equation}

We now proceed to construct an algorithm that can compute a hash for this (potentially infinite) set.
To accomplish this, we will use the following strategy, given some input graph $G$ and node $u$
\begin{enumerate}
  \item We will aggressively merge vertices in $G$ that are indistinguishable from the perspective of
  walks through the graph starting at $u$. This will result in a minimized digraph.
  \item We will show that this minimal digraph is unique.
  \item We will canonically order nodes in the minimal digraph using a relation we will define Section \ref{sec:relation}.
  \item With the ordering in hand, we can order the minimal digraph's adjacency list, which can be fed to a typical string hashing function as we have done in previous sections.
\end{enumerate}

\subsection{Minimization}

Two nodes $u$ and $v$ can be merged if any of the following conditions hold:

\begin{enumerate}
  \item $L(u) = L(v)$
  \item $L^T(u) = L^T(v)$
  \item $L(u) \supseteq L(v)$ and $L^T(u) \supseteq L^T(v)$
\end{enumerate}

\subsection{Relation}
\label{sec:relation}

Let $A$, $B$, and $C$ be sets of strings such that $A \neq B \neq C$. We define a new relation $X \prec Y \iff \min(X \triangle Y) \in Y \setminus X$ where $\triangle$ is the symmetric difference.
...
\noindent In all possible cases, $\min(A \triangle C) \in C \setminus A$, so $A \prec C$.
